# create local splunk user with random password
$NewUser = ([adsi]"WinNT://$env:COMPUTERNAME").Create('User',"splunk")
do { 
...
$NewUser.SetInfo()
$NewUser.invokeSet("userFlags", ($NewUser.userFlags[0] -BOR "65536"))
$NewUser.commitChanges()

# install the forwarder
msiexec.exe /i splunkforwarder-6.4.2-00f5bb3fa822-x64-release.msi AGREETOLICENSE=Yes LOGON_USERNAME="$env:computername\splunk" LOGON_PASSWORD="$password" SET_ADMIN_USER=0 LAUNCHSPLUNK=0 /qn

# wait for install to finish
do {
	start-sleep 1
} until ( test-path "$env:programfiles\splunkuniversalforwarder\bin\splunk.exe")

# disable rest port
new-item -path "$env:programfiles\splunkuniversalforwarder\etc\apps\UF-TA-killrest\local" -ItemType "Directory" -force | out-null
"[httpServer]`r`ndisableDefaultPort = true" | out-file "$env:programfiles\splunkuniversalforwarder\etc\apps\UF-TA-killrest\local\server.conf" -force | out-null

# set splunk admin to random password
& "$env:programfiles\splunkuniversalforwarder\bin\splunk.exe" edit user admin -password ($password) -auth admin:changeme | out-null


# need to enable user to logon as service, this does that thanks to Tony Pombo at https://gallery.technet.microsoft.com/scriptcenter/Grant-Revoke-Query-user-26e259b0
Add-Type @'
using System;
namespace PS_LSA
{
...
Grant-UserRight "splunk" SeServiceLogonRight,SeSecurityPrivilege,SeSystemProfilePrivilege,SeImpersonatePrivilege
